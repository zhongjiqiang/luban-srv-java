# luban-srv-java
## 编写服务
将所有服务实现类放到同一个package下,并在在函数之上加上注释，注释作用是生成接口文档，需严格按照给出的格式追加注释，然后编写业务逻辑，**参数都是以map对象的形式传入的**
```
	// @Title 新增会员
	// @Doc 本接口说明
...
```
## 生成api文档类
构造CreateDocJava对象传入服务实现类所在包名和Doc.java所在的路径
```
...
HproseTcpServer server = new HproseTcpServer("tcp://192.168.199.241:8088");
Services ss = new Services(docClassName);
server.add("doc", ss);

```
## 服务发布代码
服务发布代码将会在Doc.java最下面生成,用于注册服务业务
```
Base base = new Base();
